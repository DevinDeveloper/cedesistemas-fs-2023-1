...
const getAll = async (filter) => {
  try{
    ...
    if(filter.minPrice && filter.maxPrice) {
      query.price = {$gte: filter.minPrice, $lte: filter.maxPrice} // BEETWEEN
    };
    if(filter.name) query.name = { $regex: filter.name, $options: "i"}; //LIKE = i MAYUSCUALA o MINUSCILA
...
const getMyStuff = async (idUser) => {
  try{
    ...
    return clothes = await Clothe.find(query);
  }catch(error){
    throw error.handled ? error : errorHandler(dictErrors.SERVER_ERROR);
  }
}
